...
/* 喂狗间隔（秒）。0x13 WATCHDOG_TIMEOUT_SET 帧 param[2]=interval 由此宏填充：
...
struct pcat_pm_data {
	...
	/* 卸载/bind 重开窗口：置位后所有 ctl fops 入口快速失败（-ENODEV），
	 * 避免残留 fd 经 container_of 解引用已释放的 pm_data（UAF 加固）。 */
	bool cdev_removed;
	...
	const u8 *data, size_t data_len)
{
	...
	if (data_len >= 31) {
		...
		/* pcat_pm_charger_probe 失败时 pm_data->battery_info 可能为
		 * NULL，此处取消引用会触发 NULL 解引用 oops，需守卫；同时
		 * 不得整体 return，否则会跳过下方 status_report_timestamp 与
		 * RTC/温度字段缓存更新，降级态下持续刷 "Status report timeout"。
		 */
		if (pm_data->battery_info)
	...
	struct serdev_device *serdev, const u8 *buf, size_t count)
{
	...
	while (used_size < count) {
		if (pm_data->read_buffer_used + count - used_size >
			PCAT_PM_BUFFER_SIZE) {
			size_t old_used = pm_data->read_buffer_used;
			memcpy(pm_data->read_buffer + old_used,
				buf + used_size, PCAT_PM_BUFFER_SIZE - old_used);
			pm_data->read_buffer_used = PCAT_PM_BUFFER_SIZE;
			used_size += PCAT_PM_BUFFER_SIZE - old_used;
		} else {
...
static int pcat_pm_charger_probe(struct pcat_pm_data *pm_data)
{
	...
	pm_data->battery_psy = devm_power_supply_register(dev, battery_psy_desc, &pscfg);
	...
	pm_data->charger_psy = devm_power_supply_register(dev, &pcat_pm_charger_desc, &pscfg);
	...
	size_t count, loff_t *ppos)
{
	...
	if (READ_ONCE(pm_data->cdev_removed))
		return -ENODEV;
	...
	if (!pm_data->ctl_write_buffer_ready) {
		...
		ret = wait_event_interruptible(pm_data->ctl_wait,
			...
			READ_ONCE(pm_data->cdev_removed));
		if (ret)
			return ret;
		if (READ_ONCE(pm_data->cdev_removed))
			return -ENODEV;
...
static __poll_t pcat_pm_ctl_dev_poll(struct file *file, poll_table *pt)
{
	...
	/* poll 返回位掩码，无法传 -ENODEV；置位后不再等待/访问，直接返回 0 */
	if (READ_ONCE(pm_data->cdev_removed))
		return 0;
...
static int pcat_pm_probe(struct serdev_device *serdev)
{
	...
	if (device_property_read_u32(dev, "force-poweroff-timeout",
		...
		/* 规划 MCU 语义为 {60, 60, 5}，缺省 60 */
		pm_data->force_poweroff_timeout = 60;
	}
	...
	/* serdev open 失败必须 fail-stop：该函数是应用波特率/校验、创建
	 * kworker、启动 1s 心跳 hrtimer 并 arm 看门狗（0x13 帧）的唯一处。
	 * 若此处失败仍返回 0，会出现"已启动但无喂狗、看门狗未 arm"的错误
	 * 状态，因此在失败时直接返回错误码，交由 probe 失败回滚。
	 */
...
static void pcat_pm_remove(struct serdev_device *serdev)
{
	...
	/* 0) 置位 ctl 移除标志：此后任何仍打开的 fd 下一次操作（read/write/
	 * poll）都会快速失败，不再解引用将随 devm teardown 释放的 pm_data */
	WRITE_ONCE(pm_data->cdev_removed, true);
	/* 唤醒已在 read 中阻塞等待 ctl_write_buffer_ready 的读者，使其经
	 * cdev_removed 分支退出，避免线程永久挂起且不再引用将随 devm teardown
	 * 释放的 ctl_wait waitqueue（UAF 窗口） */
	wake_up_interruptible_all(&pm_data->ctl_wait);

	/* 卸载/unbind 时需在 devm teardown（释放 pm_data）之前完成清理，
	 * 否则 probe 期间 arm 的 1s hrtimer 仍会 re-arm、kworker 与
	 * /dev/pcat-pm-ctl（fops 经 container_of 引用 pm_data）会访问
	 * 已释放内存，造成 use-after-free。清理顺序如下。 */
	pm_data->work_flag = false;

	/* 1) 停止 1s 心跳 hrtimer，并取消/同步已入队的 kthread work */
	hrtimer_cancel(&pm_data->check_timer);
	if (!IS_ERR(pm_data->kworker)) {
		kthread_cancel_work_sync(&pm_data->check_work);
		/* 6.1 无 kthread_stop_worker，用配对 API kthread_destroy_worker
		 * （内部 flush worker + kthread_stop(task) + kfree(worker)）。 */
		kthread_destroy_worker(pm_data->kworker);
		pm_data->kworker = NULL;
	}

	/* 2) 释放 charger probe 分配的 battery_info（一次性泄漏修复） */
	if (pm_data->battery_info) {
		power_supply_put_battery_info(pm_data->battery_psy,
			pm_data->battery_info);
		pm_data->battery_info = NULL;
	}

	/* 3) 注销 misc 设备，避免在 pm_data 释放后 fops 仍被引用 */
