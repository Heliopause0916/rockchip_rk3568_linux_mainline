...
/*
 * Mainline 6.12 has no CONFIG_ROCKCHIP_SYSTEM_MONITOR (nor the Rockchip
 * OPP/monitor/IPA helpers this file's real implementation relies on). The
 * real devfreq implementation is kept intact under this guard for BSP/configs
 * that do enable it, and empty stub implementations are provided for mainline
 * builds (RK3568). rknpu_devfreq.o is still always linked (CONFIG_PM_DEVFREQ);
 * the stubs satisfy every symbol rknpu_drv.c calls.
 */
...
#else /* !CONFIG_ROCKCHIP_SYSTEM_MONITOR: empty stubs for mainline 6.12 */
