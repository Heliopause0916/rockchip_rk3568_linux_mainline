...
IMG_FILE="deploy/rk3568-debian-custom.img"
ROOTFS_FILE="rootfs/rootfs-debian-custom.tar.gz"
ROOTFS_BUILD_SCRIPT="mk-rootfs-debian.sh"
PARTITION_SCRIPT="scripts/photonicat-disk-parts-minimal.sfdisk"
BOOTFS_IMG_FILE="deploy/rk3568-debian-custom-bootfs.img"
ROOTFS_IMG_FILE="deploy/rk3568-debian-custom-rootfs.img"
...
tar -xpf "${ROOTFS_FILE}" --exclude="proc/*" --exclude="dev/*" --exclude="sys/*" --exclude="run/*" --xattrs --xattrs-include='*' -C "${TMP_MOUNT_DIR}/rootfs"
